...
| SAST (static analysis) | `.github/workflows/codeql.yml` (Python + JS/TS) + SonarCloud (`build.yml` `sonar-snyk` job, `sonar-project.properties`). Baseline (2026-06-26): 0 bugs, 0 security hotspots, 1 BLOCKER vulnerability -- `infra/terraform/modules/acr/main.tf` doesn't explicitly set `public_network_access_enabled` on the Container Registry (rule `terraform:S6329`). Deferred rather than fixed now: tightening it needs a private endpoint set up carefully so it doesn't break live image pulls into AKS this close to evaluation -- tracked as a known finding, see Production Roadmap |
